...
class TestGit < Test::Unit::TestCase
  ...
  def test_it_really_shell_escapes_arguments_to_the_git_shell
    @git.expects(:sh).with("#{Git.git_binary} --git-dir='#{@git.git_dir}' --work-tree='#{@git.work_tree}' foo --bar='bazz\\'er'")
    @git.foo(:bar => "bazz'er")
    @git.expects(:sh).with("#{Git.git_binary} --git-dir='#{@git.git_dir}' --work-tree='#{@git.work_tree}' bar -x 'quu\\'x'")
    @git.bar(:x => "quu'x")
  end

  def test_it_shell_escapes_the_standalone_argument
    @git.expects(:sh).with("#{Git.git_binary} --git-dir='#{@git.git_dir}' --work-tree='#{@git.work_tree}' foo 'bar\\'s'")
    @git.foo({}, "bar's")

    @git.expects(:sh).with("#{Git.git_binary} --git-dir='#{@git.git_dir}' --work-tree='#{@git.work_tree}' foo 'bar' '\\; echo \\'noooo\\''")
    @git.foo({}, "bar", "; echo 'noooo'")
  end

  def test_piping_should_work_on_1_9
    @git.expects(:sh).with("#{Git.git_binary} --git-dir='#{@git.git_dir}' --work-tree='#{@git.work_tree}' archive 'master' | gzip")
